...
const Highligts = () => {
  const some = [
    ...
    },
    {
      name: "Networking ",
      describe:
        "Ersmeronz provides opportunities to connect with like-minded individuals and industry experts. ",
      svg: (
        <svg
          viewBox="0 0 20 20"
          fill="currentColor"
    ...
    },
    {
      name: "Modern Venue",
      describe:
        " Opportunity to explore a cutting-edge Center of Excellence and well infrastructure buildings.",
      svg: (
        <svg
          viewBox="0 0 24 24"
          fill="currentColor"
  ...
  return (
    ...
    >
      ...
      As professionals, we recognize the pivotal role such events play in shaping the future of technology and fostering a culture of innovation
      </p>
      <div className=" grid px-20 max-sm:px-10 max-sm:grid-cols-1 grid-cols-3 gap-5">
        {some &&
          some.map((s) => {
            return (
              <div className="  bg rounded-3xl px-10 max-sm:px-5 py-10 border border-secondary/50 p-5 text-left flex flex-col items-start max-sm:items-center justify-center gap-3">
                ...
                <p className=" text-xl max-sm:text-center  max-sm:text-sm">{s.describe}</p>
              </div>
            );
          })}
      </div>
